...
const CryptoRTokenAddress = "0xa8186485BDFf4C9b02B12A0d566A71585f530ca5";
const CryptoRMarketplaceAddress = "0x8854Df6d69eF2B5E4d2BFDFbC0792294e90283B6";
...
$(document).ready(async function () {
  ...
  CryptoRTokenInstance = new web3.eth.Contract(
    abi.CryptoRToken,
    CryptoRTokenAddress
  );
  CryptoRMarketplaceInstance = new web3.eth.Contract(
    abi.CryptoRMarketplace,
    CryptoRMarketplaceAddress
  );
...
async function allCount() {
  ...
  let ifOfferDetailsDuplicatesRemoved = removeDuplicates(
    ifOfferDetails,
    (it) => it.tokenId
  );
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(
    (item) => !item.isSold && item.active
  ).length;
...
$("#connectWalletModalBtn").click(async () => {
  ...
  $("#connectWalletModalBtn")
    .html(`Connecting... <div class="spinner-border spinner-border-sm text-light" role="status">
                                                        <span class="sr-only">Loading...</span>
                                                      </div>`);
  //this is the one in the nav
  $("#connectWalletBtn")
    .html(`Connecting... <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                                  <span class="sr-only">Loading...</span>`);
  try {
...
function loader() {
  $(".cardDivs")
    .html(`<div id='loader' class="spinner-border mt-5 text-primary" style="width: 3rem; height: 3rem;" role="status">
                        <span class="sr-only">Loading...</span>
                      </div>`);
}
...
$("#forSale").click(async () => {
  ...
  if (activeCount < 1) {
    $(".minted-wrapper").html(
      `<div class="no-art-for-sale">There is currently no artwork for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`
    );
  }
  recentlyPutForSale();
});
...
$("#notForSale").click(async () => {
  ...
  if (inactiveCount < 1) {
    $(".minted-wrapper").html(
      `<div class="no-art-for-sale">There is currently no artwork not for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`
    );
  }
...
async function viewAll() {
  ...
  if (activeCount < 1 && inactiveCount < 1) {
    $(".minted-wrapper").html(
      `<div class="no-art-for-sale">There is currently no artwork on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`
    );
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
...
async function recentlySold() {
  try {
    ...
    let soldDetailsDuplicatesRemoved = removeDuplicates(
      soldDetails,
      (it) => it.tokenId
    );
    ...
    let restrictedToRecentEightSalesArray = soldDetailsDuplicatesRemoved.slice(
      Math.max(soldDetailsDuplicatesRemoved.length - 8, 0)
    );
    ...
    for (i = 0; i < restrictedToRecentEightSalesArray.length; i++) {
      ...
      let unlockableContent =
        restrictedToRecentEightSalesArray[i].unlockableContent;
      ...
      if (unlockableContent) {
        $("#soldCard" + tokenAddress + id).addClass(
          "unlockable-content-shadow"
        );
      }
      ...
      $("#soldCardNotForSale" + tokenAddress + id).html(
        `Sold For: <br><span class="for-sale-text">${priceInEth} BNB</span>`
      );
      $("#soldCardButton" + tokenAddress + id).html(
        `<a href="./token.html?token=` +
          tokenAddress +
          id +
          `"><button class="btn btn-light view-btn">View</button></a>`
      );
...
function dismissLoadingPulseOnSoldCover(tokenAddress, id, cover) {
  ...
  img.onerror = function () {
    $("#nftsoldCardSpinnerGrowSpinner" + tokenAddress + id).css(
      "display",
      "none"
    );
    $("#soldCardCover" + tokenAddress + id).css("display", "block");
    $("#soldCardCover" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/cantFindNFT.png"
    );
  ...
  try {
    ...
    for (i = 0; i < users.length; i++) {
      ...
      if (ethAddress == owner && profilePhoto) {
        ...
        $("#soldCardOwnerPhoto" + tokenAddress + id).attr(
          "src",
          profilePhoto._url
        );
        dismissLoadingPulseOnSoldCardOwnerPhoto(
          tokenAddress,
          id,
          profilePhoto._url
        );
...
function addSoldCardSellerBadge(tokenAddress, id, amountSold) {
  if (amountSold == undefined) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/noSales.png"
    );
  } else if (amountSold >= 1 && amountSold <= 4) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/oneSale.png"
    );
  } else if (amountSold >= 5 && amountSold <= 9) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/fiveSales.png"
    );
  } else if (amountSold >= 10 && amountSold <= 19) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/tenSales.png"
    );
  } else if (amountSold >= 20 && amountSold <= 34) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/twentySales.png"
    );
  } else if (amountSold >= 35 && amountSold <= 49) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/thirtyfiveSales.png"
    );
  } else if (amountSold >= 50 && amountSold <= 74) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/fiftySales.png"
    );
  } else if (amountSold >= 75 && amountSold <= 99) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/seventyfiveSales.png"
    );
  } else if (amountSold >= 100) {
    $("#soldCardOwnerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/hundredPlusSales.png"
    );
  }
}

function dismissLoadingPulseOnSoldCardOwnerPhoto(
  tokenAddress,
  id,
  profilePhoto
) {
  ...
  img.onerror = function () {
    ...
    $("#soldCardOwnerPhoto" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/cantFindProfilePhoto.png"
    );
  };
}
...
function soldCardFileIcon(tokenAddress, id, fileType) {
  if (
    ...
    $("#soldCardFileIcon" + tokenAddress + id).attr(
      "src",
      "assets/images-icons/videoIcon.png"
    );
  } else if (
    fileType == "audio/mp3" ||
    fileType == "audio/webm" ||
    ...
    $("#soldCardFileIcon" + tokenAddress + id).attr(
      "src",
      "assets/images-icons/audioIcon.png"
    );
  }
}
...
function recentlySoldQuickAction(tokenAddress, id, owner) {
  $("#soldCardQuickActions" + tokenAddress + id).html(
    `<a class="dropdown-item quick-action" id="soldCardShareQuickAction` +
      tokenAddress +
      id +
      `" data-toggle="modal" data-target="#soldCardShareModal` +
      tokenAddress +
      id +
      `">Share</a>`
  );
...
async function recentlyPutForSale() {
  try {
    let ifOfferDetails = await Moralis.Cloud.run("getOfferDetails");
    let ifOfferDetailsLength = removeDuplicates(
      ifOfferDetails,
      (art) => art.tokenId
    ).length;
    let ifOfferDetailsDuplicatesRemoved = removeDuplicates(
    ...
    for (i = 0; i < ifOfferDetailsLength; i++) {
      if (
        ifOfferDetailsDuplicatesRemoved[i].active == true &&
        ifOfferDetailsDuplicatesRemoved[i].isSold == false
      ) {
        ...
        let unlockableContent =
          ifOfferDetailsDuplicatesRemoved[i].unlockableContent;
        ...
        let priceInEth = web3.utils.fromWei(price, "ether");
        $("#forSale" + tokenAddress + id).html(
          `<span class="for-sale-text">${priceInEth} BNB</span>`
        );
        $("#button" + tokenAddress + id).html(
          `<a href="./token.html?token=` +
            tokenAddress +
            id +
            `"><button class="btn btn-primary buy-btn">Buy</button></a>`
        );
      }
      darkmodeForDynamicContent();
    }
  ...
  try {
    ...
    for (i = 0; i < inactiveArtwork.length; i++) {
      if (inactiveArtwork[i].active == false) {
        ...
        $("#name" + tokenAddress + id).html(name);
        $("#notForSale" + tokenAddress + id).html(
          `<button id="encourageBell` +
            tokenAddress +
            id +
            `" class="btn like-encourage-button fas fa-concierge-bell"><span class="like-encourage-text" id="encourageCounter` +
            tokenAddress +
            id +
            `"></span></button>`
        );
...
async function showBellsFilled(tokenAddress, id) {
  if (user) {
    ...
    let likeQuery = await Moralis.Cloud.run(
      "userEncouragedThisArtwork",
      params
    );
    if (likeQuery) {
...
function encourageButton(tokenAddress, id) {
  $("#encourageBell" + tokenAddress + id).click(async () => {
    if (user) {
      ...
      let encourageQuery = await Moralis.Cloud.run(
        "userEncouragedThisArtwork",
        params
      );
      if (encourageQuery) {
  ...
  try {
    ...
    for (i = 0; i < users.length; i++) {
      ...
      if (ethAddress == owner && profilePhoto) {
        ...
        addSellerBadge(tokenAddress, id, amountSold);
        $("#ownerPhoto" + tokenAddress + id).attr(
          "src",
          "./assets/images-icons/default.png"
        );
        let defaultProfilePhoto = "./assets/images-icons/default.png";
...
function addSellerBadge(tokenAddress, id, amountSold) {
  if (amountSold == undefined) {
    $("#ownerRank" + tokenAddress + id).attr(
      "src",
      "./assets/images-icons/noSales.png"
    );
  } else if (amountSold >= 1 && amountSold <= 4) {
...
function quickActions(tokenAddress, id, owner, active, royalty, creator) {
  if (
    user == null ||
    user.attributes.ethAddress.toLowerCase() !== owner.toLowerCase()
  ) {
    $("#quickActions" + tokenAddress + id).html(
      `<a class="dropdown-item quick-action" id="shareQuickAction` +
        tokenAddress +
        id +
    ...
    $("#quickActions" + tokenAddress + id).html(
      ` <a class="dropdown-item quick-action" id="changePriceQuickAction` +
        tokenAddress +
        id +
        ...
        id +
        `">Change price</a>
                                                  <a class="dropdown-item quick-action" id="removeFromSaleQuickAction` +
        tokenAddress +
        id +
        `" data-toggle="modal" data-target="#removeFromSaleModal` +
        tokenAddress +
        id +
        `">Remove from sale</a>
                                                  <a class="dropdown-item quick-action" id="shareQuickAction` +
    ...
    $("#quickActions" + tokenAddress + id).html(
      ` <a class="dropdown-item quick-action" id="putForSaleQuickAction` +
        tokenAddress +
        id +
        `" data-toggle="modal" data-target="#putForSaleModal` +
        tokenAddress +
        id +
        `">Put for sale</a>
                                                  <a class="dropdown-item quick-action" id="transferTokenQuickAction` +
        tokenAddress +
        id +
        `" data-toggle="modal" data-target="#transferTokenModal` +
        tokenAddress +
        id +
        `">Transfer token</a>
                                                  <a class="dropdown-item quick-action" id="shareQuickAction` +
...
async function checkForApprovalBeforePuttingOnSale(tokenAddress, id) {
  try {
    let approved = await CryptoRTokenInstance.methods
      .isApprovedForAll(user.attributes.ethAddress, CryptoRMarketplaceAddress)
      .call();
...
function setApprovalIfNeeded(tokenAddress, id) {
  $("#setApprovalBtn" + tokenAddress + id).click(async () => {
    ...
    $("#setApprovalBtn" + tokenAddress + id)
      .html(`Setting Approval <div class="spinner-border spinner-border-sm text-light" role="status">
                                <span class="sr-only">Loading...</span>
                              </div>`);
    await CryptoRTokenInstance.methods
      .setApprovalForAll(CryptoRMarketplaceAddress, true)
      .send({ from: user.attributes.ethAddress }, (err, txHash) => {
        if (err) {
          alert(err.message);
          ...
          $("#setApprovalBtn" + tokenAddress + id).html(
            "Approval Successfully Granted"
          );
          $("#salePriceInput" + tokenAddress + id).prop("disabled", false);
        }
      });
...
function putOnSale(tokenAddress, id, royalty, creator) {
  $("#putOnSaleBtn" + tokenAddress + id).click(async () => {
    ...
    $("#putOnSaleBtn" + tokenAddress + id).prop("disabled", true);
    $("#putOnSaleBtn" + tokenAddress + id)
      .html(`Put On Sale <div class="spinner-border spinner-border-sm text-light" role="status">
                                                  <span class="sr-only">Loading...</span>
                                                </div>`);
    try {
      await CryptoRMarketplaceInstance.methods
        .setOffer(amountInWei, id, tokenAddress)
      ...
      $("#forSale" + tokenAddress + id).html(
        `<span class="for-sale-text">${price} BNB</span>`
      );
      $("#button" + tokenAddress + id).html(
        `<a href="./token.html?token=` +
          tokenAddress +
...
function putForSaleInput(tokenAddress, id, royalty, creator) {
  $("#salePriceInput" + tokenAddress + id).keyup(async () => {
    ...
    if (creator == user.attributes.ethAddress) {
      ...
      let profit = price - price * 0.02 - price * (royalty / 100);
      $("#saleProfit" + tokenAddress + id).html(`${profit} BNB`);
      let usdProfit = (profit * ethPrice).toFixed(2);
      $("#usdProfit" + tokenAddress + id).html(`$${usdProfit}`);
    }
...
function removeFromSale(tokenAddress, id, royalty, creator) {
  ...
  $("#removeFromSaleBtn" + tokenAddress + id).click(async () => {
    $("#removeFromSaleBtn" + tokenAddress + id).prop("disabled", true);
    $("#removeFromSaleBtn" + tokenAddress + id)
      .html(`Remove From Sale<div class="spinner-border spinner-border-sm text-light" role="status">
                                                        <span class="sr-only">Loading...</span>
                                                      </div>`);
    try {
      await CryptoRMarketplaceInstance.methods
        .removeOffer(id, tokenAddress)
        .send({ from: user.attributes.ethAddress });
...
function changePriceFrontEnd(tokenAddress, id, royalty, creator) {
  $("#changePriceBtn" + tokenAddress + id).click(async () => {
    ...
    $("#changePriceBtn" + tokenAddress + id).prop("disabled", true);
    $("#changePriceBtn" + tokenAddress + id)
      .html(`Change Price <div class="spinner-border spinner-border-sm text-light" role="status">
                                                    <span class="sr-only">Loading...</span>
                                                  </div>`);
    try {
      await CryptoRMarketplaceInstance.methods
        .changePrice(amountInWei, id, tokenAddress)
        .send({ from: user.attributes.ethAddress });
      $("#changePriceBtn" + tokenAddress + id).html(
        "Successfully Changed Price"
      );
...
function transferToken(tokenAddress, id) {
  $("#transferTokenBtn" + tokenAddress + id).click(async () => {
    ...
    $("#transferTokenBtn" + tokenAddress + id).prop("disabled", true);
    $("#transferTokenBtn" + tokenAddress + id)
      .html(`Transfer Token <div class="spinner-border spinner-border-sm text-light" role="status">
                                                        <span class="sr-only">Loading...</span>
                                                      </div>`);
    try {
      await CryptoRTokenInstance.methods
        .safeTransferFrom(user.attributes.ethAddress, toAddress, id)
        .send({ from: user.attributes.ethAddress });
      $("#transferTokenBtn" + tokenAddress + id).html(
      ...
      $("#owner" + tokenAddress + id).attr(
        "href",
        "./profile.html?address=" + toAddress
      );
...
function toAddressInput(tokenAddress, id) {
  $("#toAddressInput" + tokenAddress + id).keyup(() => {
    ...
    if (address.toLowerCase() == user.attributes.ethAddress.toLowerCase()) {
      ...
      $("#regexMessage" + tokenAddress + id).html(
...
function soldCardDiv(tokenAddress, id, owner) {
  let nftSoldCard =
    ...
    tokenAddress +
    id +
    `" class="grid-helper col-10 col-sm-6 col-md-4 col-lg-3 col-xl-2">
                      <div id="soldCard` +
    tokenAddress +
    id +
    `" class="card sold-item shadow-sm">
                        <div class="top-row">
                          <div class="creator-photo">
                            <a href="./profile.html?address=` +
    owner +
    `"><img loading="lazy" class="owner shadow-sm" id="soldCardOwnerPhoto` +
    tokenAddress +
    id +
    `" src="" width="40" alt="owner photo">
                              <span id="soldcardSpinner` +
    tokenAddress +
    id +
    `" class="spinner-grow text-light" style="width: 40px; height: 40px; margin: 0; padding: 0;" role="status">
                                <span class="sr-only">Loading...</span>
                              </span>
                              <div class="rank-badge">
                                <img id="soldCardOwnerRank` +
    tokenAddress +
    id +
    `" src="" width="15" alt="badge based on how many sales from account">
                              </div>
                            </a>
                          </div>
                          ...
                          <div class="dropleft">
                            ...
                            <div class="dropdown-menu" id="soldCardQuickActions` +
    tokenAddress +
    id +
    ...
    id +
    `">
                            <span id='soldCardSpinnerGrow` +
    tokenAddress +
    id +
    `' class="spinner-grow text-light embed-responsive-item" role="status">
                              <span class="sr-only">Loading...</span>
                            </span>
                            <img loading="lazy" id='soldCardCover` +
    tokenAddress +
    id +
    `' src="" class="card-img embed-responsive-item" alt="">
                            <span class="file-indicator"><img id="soldCardFileIcon` +
    tokenAddress +
    id +
    `" src="" width="20"></span>
                          </a>
                        ...
                        <div class="card-body">
                          <a class="anchor" href="./token.html?token=` +
    tokenAddress +
    id +
    `">
                            <p id="soldCardName` +
    ...
    `" class="card-title"></p>
                          ...
                          <p class="card-text sold-card-text" id="soldCardNotForSale` +
    tokenAddress +
    id +
    `"></p>
                          <div class="button-row">
                            <button class="btn btn-light like-encourage-button far fa-heart heart" id="soldCardLike` +
    tokenAddress +
    id +
    `">
                              <span class="like-encourage-text like-counter" id="soldCardLikeCounter` +
    tokenAddress +
    id +
    `"> </span>
                            </button>
                            <span id="soldCardButton` +
    tokenAddress +
    id +
    `"></span>
                          </div>
...
function cardDiv(tokenAddress, id, owner) {
  let nftCard =
    ...
    tokenAddress +
    id +
    `" class="grid-helper col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-3">
                    <div id="card` +
    tokenAddress +
    id +
    `" class="card minted-item shadow-sm">
                      <div class="top-row">
                        <div class="creator-photo">
                          <a id='owner` +
    tokenAddress +
    id +
    `' href="./profile.html?address=` +
    owner +
    `"><img loading="lazy" class="owner shadow-sm" id="ownerPhoto` +
    tokenAddress +
    id +
    `" src="" width="40" alt="owner photo">
                            <span id="cardSpinner` +
    ...
    tokenAddress +
    id +
    `" src="" width="15" height="15" alt="seller badge">
                            </div>
                          </a>
                        </div>
                        <!--NOT owner AND it's NOT on market these will be the options-->
                        <div class="dropleft">
                          ...
                          <div class="dropdown-menu" id="quickActions` +
    tokenAddress +
    id +
    `" aria-labelledby="dropdownMenuButton">
                          </div>
    ...
    `' class="spinner-grow text-light embed-responsive-item" role="status">
                          ...
                          <img loading="lazy" id='cover` +
    tokenAddress +
    id +
    `' src="" class="card-img embed-responsive-item" alt="">
                          <span class="file-indicator"><img id="fileIcon` +
    tokenAddress +
    id +
    `" src="" width="20"></span>
                        </a>
    ...
    `" class="card-title"></p>
                        ...
                        <p class="card-text" id="forSale` +
    tokenAddress +
    id +
    `"></p>
                        <p class="card-text not-for-sale-text" id="notForSale` +
    tokenAddress +
    id +
    `"></p>
                        <div class="button-row">
                          <button class="btn btn-light like-encourage-button far fa-heart heart" id="like` +
    tokenAddress +
    id +
    `">
                            <span class="like-encourage-text like-counter" id="likeCounter` +
    tokenAddress +
    id +
    `"> </span>
                          </button>
...
function changePriceModalHTML(tokenAddress, id) {
  let changePriceModal =
    `<div class="modal fade" id="changePriceModal` +
    tokenAddress +
    id +
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form>
                        <div id="changePriceInputGroup` +
    tokenAddress +
    id +
    `" class="price-input-group">
                          <div class="input-group">
                            <input id="changePriceInput` +
    tokenAddress +
    id +
    `" type="text" class="form-control input-styling" placeholder="Enter price in BNB" aria-label="ether amount">
                          </div>
                          <div class="price-calculator price-info">
                            <span>Service Fee Upon Sale <span>2%</span></span><br>
                            <span id="ifOwnerNotCreator` +
    tokenAddress +
    id +
    `">Creator's Royalty <span id="royalty` +
    tokenAddress +
    id +
    `"></span><br></span>
                            <span>Your profit will be: <span id="changePriceSaleProfit` +
    tokenAddress +
    id +
    `" class="sale-profit">0 BNB</span> <span id="changePriceUSDProfit` +
    tokenAddress +
    id +
    `">$0.00</span></span>
                          </div>
                    ...
                    <div class="modal-footer change-price-footer">
                      ...
                      <button type="button" class="btn btn-primary button-styling" disabled id="changePriceBtn` +
    tokenAddress +
    id +
    `">Change Price</button>
                    </div>
...
function putForSaleModalHTML(tokenAddress, id) {
  let putForSaleModal =
    `<div class="modal fade" id="putForSaleModal` +
    tokenAddress +
    id +
    `" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <button type="button" class="btn btn-primary btn-block button-styling" id="setApprovalBtn` +
    tokenAddress +
    id +
    `">Set Approval To Sell</button>
                              <form>
    ...
    id +
    `"></span><br></span>
                                    <span>Your profit will be: <span id="saleProfit` +
    tokenAddress +
    id +
    `" class="sale-profit">0 BNB</span> <span id="usdProfit` +
    tokenAddress +
    id +
    `">$0.00</span></span>
                                  </div>
                            ...
                            <div class="modal-footer">
                              <button type="button" class="btn btn-secondary button-styling" data-dismiss="modal">Close</button>
                              <button type="button" class="btn btn-primary button-styling" disabled id="putOnSaleBtn` +
    tokenAddress +
    id +
    `">Put For Sale</button>
                            </div>
...
function removeFromSaleModalHTML(tokenAddress, id) {
  let removeFromSaleModal =
    `<div class="modal fade" id="removeFromSaleModal` +
    tokenAddress +
    id +
    `" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
                              <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <button type="button" class="btn btn-primary btn-block button-styling" id="yesContinueBtn` +
    tokenAddress +
    id +
    `">Yes, continue</button>
                                  </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary button-styling" data-dismiss="modal">Close</button>
                                    <button type="button" class="btn btn-danger button-styling" disabled id="removeFromSaleBtn` +
    tokenAddress +
    id +
...
function transferTokenModalHTML(tokenAddress, id) {
  let transferTokenModal =
    ` <div class="modal fade" id="transferTokenModal` +
    tokenAddress +
    id +
    `" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
                              <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body">
                                    <form>
                                      <div id="toAddressInputGroup` +
    tokenAddress +
    id +
    `" class="price-input-group">
                                        <div class="input-group">
                                          <input id="toAddressInput` +
    tokenAddress +
    id +
    `" type="text" class="form-control input-styling" placeholder="0x..." aria-label="receiver address">
                                        </div>
                                        <div class="price-info">
                                          <span>Enter the address where you want to send this artwork</span><br>
                                          <span id="regexMessage` +
    tokenAddress +
    id +
    `"></span>
                                        </div>
                                  ...
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary button-styling" data-dismiss="modal">Close</button>
                                    <button type="button" class="btn btn-primary button-styling" disabled id="transferTokenBtn` +
    tokenAddress +
    id +
    ...
    `" tabindex="-1" role="dialog" aria-hidden="true">
                              <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                  ...
                                  <button id='twitterBtnInModal` +
    tokenAddress +
    id +
    `' target="popup" type="button" class="btn btn-primary m-2 button-styling">Twitter</button>
                                  <button id='facebookBtnInModal` +
    tokenAddress +
    id +
    `' target="popup" type="button" class="btn btn-primary m-2 button-styling">Facebook</button>
                                  <button id='emailBtnInModal` +
    tokenAddress +
    id +
    `' type="button" class="btn btn-primary m-2 button-styling">Email</button>
                                  </div>
                                </div>
                              </div>
                            </div>`;
...
function soldCardShareModalHTML(tokenAddress, id) {
  let soldCardShareModal =
    `<div class="modal fade" id="soldCardShareModal` +
    tokenAddress +
    id +
    `" tabindex="-1" role="dialog" aria-hidden="true">
                              <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body center-content">
                                  <button id='soldCardTwitterBtnInModal` +
    tokenAddress +
    id +
    `' target="popup" type="button" class="btn btn-primary m-2 button-styling">Twitter</button>
                                  <button id='soldCardFacebookBtnInModal` +
    tokenAddress +
    id +
    `' target="popup" type="button" class="btn btn-primary m-2 button-styling">Facebook</button>
                                  <button id='soldCardEmailBtnInModal` +
    tokenAddress +
    id +
    `' type="button" class="btn btn-primary m-2 button-styling">Email</button>
                                  </div>
