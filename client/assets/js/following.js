...
function isUserIn() {
  if (!user) {
    $(".following").html(
      '<p class="center-content not-connected-text">Please connect wallet to view artwork from artists you follow</p>'
    );
...
async function checkIfUserIsFollowingPeople() {
  ...
  if (followingArray == undefined || !followingArray.length) {
    $(".following").html(
      "<div class='center-content'><span class='not-connected-text'>Follow an artist to see the NFTs they create here.<br><span class='price-info'>The follow button is located on the user's profile page</span></span><div>"
    );
  } else {
...
async function allCount() {
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(
    (item) =>
      !item.isSold && item.active && followingArray.includes(item.creator)
  ).length;
  let inactiveCount = inactiveArtwork.filter(
    (item) => !item.active && followingArray.includes(item.creator)
  ).length;
...
$("#forSale").click(async () => {
  ...
  if (activeCount < 1) {
    $(".following-wrapper").html(
      `<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork for sale on Open<span class="gradient-text">Mint</span> at the moment.<div>`
    );
  }
  recentlyPutForSale();
});
...
$("#notForSale").click(async () => {
  ...
  if (inactiveCount < 1) {
    $(".following-wrapper").html(
      `<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork not for sale on Open<span class="gradient-text">Mint</span> at the moment.<div>`
    );
  }
...
async function viewAll() {
  ...
  if (activeCount < 1 && inactiveCount < 1) {
    $(".following-wrapper").html(
      `<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork created on Open<span class="gradient-text">Mint</span> at the moment.<div>`
    );
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
  ...
  try {
    ...
    for (i = 0; i < ifOfferDetailsLength; i++) {
      if (
        ...
        followingArray.includes(ifOfferDetailsDuplicatesRemoved[i].creator)
      ) {
...
async function recentlyMintedAndNotOnSale() {
  try {
    ...
    for (i = 0; i < inactiveArtwork.length; i++) {
      if (
        inactiveArtwork[i].active == false &&
        followingArray.includes(inactiveArtwork[i].creator)
      ) {
...
function putForSaleModalHTML(tokenAddress, id) {
  let putForSaleModal =
    ...
    tokenAddress +
    id +
    `" type="text" class="form-control input-styling" placeholder="Enter price in ETH" aria-label="ether amount">
                                  </div>
